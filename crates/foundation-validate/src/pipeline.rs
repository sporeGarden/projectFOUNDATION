...
impl PipelineConfig {
    ...
    pub fn from_project_root(root: PathBuf) -> Self {
        Self {
            ...
            gate_name: std::env::var("GATE_NAME").unwrap_or_else(|_| String::from("irongate")),
            thread_filter: None,
            skip_fetch: false,
            project_root: root,
        }
...
impl ValidationPipeline {
    ...
    pub async fn run(&self) -> Result<ValidationResult, Box<dyn std::error::Error>> {
        ...
        let _discovery_config = DiscoveryConfig::from_file(&self.config.discovery_config_path)?;
        ...
        // Phase 1: Health checks (IPC-dependent — degrade gracefully)
        let health_summary = String::from("degraded: IPC composition pending");

        // Phase 2: Provenance session (IPC-dependent)
        let provenance_summary = String::from("degraded: provenance composition pending");

        // Determine which threads to process
        let threads = resolve_threads(&thread_index, self.config.thread_filter.as_deref());

        // Phase 3: Fetch sources (local-only for now)
        ...
        // Phase 7: Provenance commit (IPC-dependent)
        // Will be wired when rhizoCrypt/loamSpine composition is available
        ...
        let overall_pass = comparison.as_ref().is_none_or(ComparisonReport::all_passed);
    ...
    fn run_fetch_phase(&self, threads: &[String]) -> Vec<(String, bool)> {
        ...
        let sources_dir = self.config.project_root.join("data/sources");

        for thread_short in threads {
            let source_file = sources_dir.join(format!("{thread_short}.toml"));
    ...
    fn run_execution_phase(&self, threads: &[String]) -> Vec<ExecutionResult> {
        let mut results = Vec::new();
        let workloads_dir = self.config.project_root.join("workloads");

        for thread_short in threads {
            let thread_dir = workloads_dir.join(thread_short);
    ...
    fn run_comparison_phase(
        ...
        let targets_dir = self.config.project_root.join("data/targets");
        let mut all_observations: Vec<(String, f64)> = Vec::new();
        let mut combined_manifest: Option<TargetsManifest> = None;

        // Gather observations from successful executions
        ...
        for thread_short in threads {
            let targets_file = targets_dir.join(format!("{thread_short}_targets.toml"));
...
fn resolve_threads(index: &ThreadIndex, filter: Option<&str>) -> Vec<String> {
    filter.map_or_else(
        || index.threads.iter().map(|t| t.short.clone()).collect(),
        |f| {
